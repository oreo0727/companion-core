...
step "Generating Companion operating-system routines"
MORNING_RUN="$(http_post_json "${API_URL}/api/os/routines/MorningStartup/generate" '{}')"
assert_json "$MORNING_RUN" "data['run']['routineType'] == 'MorningStartup' and data['run']['scheduledAgentRunId'] is not None and len(data['scheduledAgentRuns']) == 1" "Morning startup routine schedules background AgentRun"
EVENING_RUN="$(http_post_json "${API_URL}/api/os/routines/EveningRecap/generate" '{}')"
assert_json "$EVENING_RUN" "data['run']['routineType'] == 'EveningRecap' and data['run']['forecastJson'] != ''" "Evening recap routine includes forecast"
WEEKLY_RUN="$(http_post_json "${API_URL}/api/os/routines/WeeklyReview/generate" '{}')"
assert_json "$WEEKLY_RUN" "data['run']['routineType'] == 'WeeklyReview' and data['run']['insightsJson'] != '[]'" "Weekly review routine includes proactive insights"
CONTEXT_RUN="$(http_post "${API_URL}/api/os/context/optimize")"
assert_json "$CONTEXT_RUN" "data['run']['routineType'] == 'ContextOptimization' and data['run']['actionsJson'] != '[]'" "Context optimization routine includes actions"
OS_RUNS="$(http_get "${API_URL}/api/os/runs")"
assert_json "$OS_RUNS" "sum(1 for x in data if x['routineType'] in ['MorningStartup','EveningRecap','WeeklyReview','ContextOptimization']) >= 4" "Operating-system runs persist"
OS_AUDIT="$(http_get "${API_URL}/api/audit")"
